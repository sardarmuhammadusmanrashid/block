...
const Blood = ({ state }) => {
  ...
  const total1e = async () => {
    ...
    try {
      ...
      const balance = await contract.methods.sendproblem(email,name1,"1234567").send({ from: account});
      // const balanced1 = await contract.methods.result().call();
      // console.log(balance);
      setname111("SignUp please login");
...
<button className="btn btn-outline-primary" onClick={total1e}>SIgnup</button>&nbsp;&nbsp;
